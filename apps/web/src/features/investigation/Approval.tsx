...
export function ApprovalPanel({
  ...
  return (
    <motion.section
      ...
      className={cn(
        'rounded-[--radius-panel] border-2 bg-[--color-surface]',
        decided
          ? 'border-[--color-line]'
    ...
    >
      ...
      >
        {decided ? (
          <ShieldCheck size={15} className="text-[--color-ink-muted]" />
        ) : (
          <AlertOctagon size={15} className="text-[--color-state-review]" />
        )}
        <h2
          className={cn(
            'text-[0.78rem] font-semibold uppercase tracking-[0.08em]',
            decided ? 'text-[--color-ink-muted]' : 'text-[--color-state-review]',
          )}
        >
      ...
      <div className="space-y-4 p-4">
        <div>
          <h3 className="text-base font-semibold text-[--color-ink]">{remediation.title}</h3>
        ...
        <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-3">
          <div>
            <dt className="text-[0.68rem] uppercase tracking-[0.07em] text-[--color-ink-subtle]">
              Evidence
            </dt>
          ...
          <div>
            ...
            <dd className="mt-1 text-sm text-[--color-ink]">{remediation.affectedFiles.length}</dd>
          </div>
          <div>
            <dt className="text-[0.68rem] uppercase tracking-[0.07em] text-[--color-ink-subtle]">
              Approved by
        ...
        <div>
          <p className="text-[0.68rem] uppercase tracking-[0.07em] text-[--color-ink-subtle]">
            Expected impact
          </p>
        ...
        <div>
          ...
          {showDiff ? (
            <pre className="mono mt-3 max-h-80 overflow-auto rounded-md border border-[--color-line] bg-[--color-surface-sunken] p-3 text-[0.72rem] leading-relaxed">
              {remediation.diff.split('\n').map((line, index) => (
                <div
                  key={index}
                  className={cn(
                    line.startsWith('+') && !line.startsWith('+++') && 'text-[--color-state-resolved]',
                    line.startsWith('-') && !line.startsWith('---') && 'text-[--color-state-severe]',
                    line.startsWith('@@') && 'text-[--color-state-active]',
                    line.startsWith('diff --git') && 'mt-2 font-semibold text-[--color-ink]',
                  )}
                >
                  {line || ' '}
                </div>
        ...
        {error ? (
          <p className="rounded-md border border-[color-mix(in_oklch,var(--color-state-severe)_40%,transparent)] bg-[color-mix(in_oklch,var(--color-state-severe)_10%,transparent)] px-3 py-2 text-xs text-[--color-state-severe]">
            {error}
          </p>
        ) : null}
        ...
        {!decided ? (
          rejecting ? (
            <div className="space-y-2">
              <label htmlFor="reject-reason" className="block text-xs text-[--color-ink-muted]">
              ...
              <textarea
                ...
                rows={2}
                className="w-full rounded-md border border-[--color-line] bg-[--color-surface-sunken] px-3 py-2 text-sm text-[--color-ink] placeholder:text-[--color-ink-subtle]"
